...
public class Listeners {
	...
	static ActionListener writeFileListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			} catch (ParserConfigurationException | TransformerFactoryConfigurationError | TransformerException e1) {
				e1.printStackTrace();
			}
		}
	};
	...
	static ActionListener searchRowsListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			Data.sportsmenList.forEach(s -> {
				if ((!FIO.isEmpty() || !sport.isEmpty()) && titlesFrom.isEmpty() && titlesTo.isEmpty()
						&& category.isEmpty()) {
					if (FIO.equals(s.getFIO()) || sport.equals(s.getSport()))
						searchResult.add(s);
				} else if (!titlesFrom.isEmpty() && !titlesTo.isEmpty() && category.isEmpty() && FIO.isEmpty()
						&& sport.isEmpty()) {
					if (s.getTitles() > Integer.parseInt(titlesFrom) && s.getTitles() < Integer.parseInt(titlesTo))
						searchResult.add(s);
				} else if ((!FIO.isEmpty() || !category.isEmpty()) && titlesFrom.isEmpty() && titlesTo.isEmpty()
						&& sport.isEmpty()) {
			...
			if (!wrongInput) {
				...
				JOptionPane.showMessageDialog(SearchDialog.searchDialog,
	...
	static ActionListener deleteRowsListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (!wrongInput) {
				PageController.refreshTableModel();
				if (counter == 0)
					JOptionPane.showMessageDialog(DeleteDialog.deleteDialog,
